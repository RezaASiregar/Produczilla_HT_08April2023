...
const omzet = data.map((d) => {
    return {
        idTransaction: d.id,
        revenue: d.items.reduce((total, item) => {
    ...
    .sort((transactionA, transactionB) => transactionA.revenue - transactionB.revenue);

  omzet.lowest = {
    idTransaction: omzet[0].idTransaction,
    revenue: `USD ${omzet[0].revenue.toFixed(2)}`,
  };

  omzet.highest = {
    idTransaction: omzet[omzet.length - 1].idTransaction,
    revenue: `USD ${omzet[omzet.length - 1].revenue.toFixed(2)}`,
  };

  omzet.total = `USD ${omzet
    .reduce((total, transaction) => {
      return total + transaction.revenue;
    }, 0)
    .toFixed(2)}`;
